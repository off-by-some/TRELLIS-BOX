...
from trellis.utils.device import autocast_context, configure_torch_threads, empty_device_cache, get_trellis_device, is_cuda_runtime, synchronize_device
...
class ModelGenerator:
    ...
    @staticmethod
    def pack_model_state(gs: Optional[Gaussian], mesh: MeshExtractResult, trial_id: str) -> Dict[str, Any]:
        """
    ...
    @staticmethod
    def unpack_model_state(state: Dict[str, Any]) -> Tuple[Optional[Gaussian], edict, str]:
        """
        ...
        """
        gs = None
        if state.get('gaussian') is not None:
            gs = Gaussian(
        ...
        mesh = edict(
            ...
            vertex_attrs=state['mesh'].get('vertex_attrs'),
        )
    ...
    def generate_from_single_image(
        ...
        pipeline = StateManager.get_pipeline()
        output_formats = ["gaussian", "mesh"] if is_cuda_runtime() else ["mesh"]
        ...
        # Extract generated mesh. CUDA mode also keeps Gaussian data for previews/textured export.
        ...
        video_path = None
        if is_cuda_runtime() and gaussian is not None:
            ...
            video = render_utils.render_video(mesh, resolution=384, num_frames=36)
            video_path = f"{TMP_DIR}/{video_trial_id}.mp4"
            os.makedirs(os.path.dirname(video_path), exist_ok=True)
            VideoRenderer.create_side_by_side_video(video['color'], video['normal'], video_path, fps=12, quality=7)
            del video
            gc.collect()
    ...
    def generate_from_multiple_images(
        ...
        with torch.inference_mode():
            ...
            outputs = pipeline.decode_slat(slat, output_formats)
            del slat
...
class GLBExporter:
    ...
    @staticmethod
    def _export_mesh_only_glb(mesh: edict, glb_path: str) -> None:
    ...
    def extract(state: Dict[str, Any], params: ExportParams) -> Tuple[str, str]:
        ...
        if gs is None or not is_cuda_runtime():
            GLBExporter._export_mesh_only_glb(mesh, glb_path)
            glb = None
        else:
            # Generate textured GLB (postprocessing uses CUDA rasterization)
...
class SingleImageUI:
    ...
    def _render_output_preview(video_key: str, glb_key: str, download_key: str) -> None:
        ...
        # CUDA mode provides both preview video and GLB. Portable CPU/macOS mode
        # skips the CUDA preview but can still produce a mesh-only GLB.
        if generated_glb and generated_state:
...
class TrellisApp:
    ...
    def _check_gpu() -> Optional[str]:
        """
        Check runtime availability and return runtime info.
        ...
        if device.type != "cuda":
            st.warning(
                f"Running TRELLIS on {device.type.upper()}. This portable mode is experimental, "
                "mesh-only, and can be very slow."
            )
            return f"{device.type.upper()} portable mode"
    ...
    def _initialize_pipeline(self) -> None:
        ...
        if pipeline is None:
            ...
            # Clean device memory before loading
            if is_cuda_runtime():
