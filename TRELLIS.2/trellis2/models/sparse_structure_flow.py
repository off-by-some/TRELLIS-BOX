...
class SparseStructureFlowModel(nn.Module):
    ...
    def prepare_inference_block_offload(self, device: torch.device, offload_device: torch.device = torch.device("cpu")) -> None:
        """
        Keep only non-transformer shell modules resident on the target device.

        The transformer blocks are moved to the target device one at a time
        during no-grad inference, preserving the same weights and math while
        lowering peak VRAM at the cost of CPU/GPU transfer time.
        """
        device = torch.device(device)
        offload_device = torch.device(offload_device)
        self.inference_block_offload = True
        self.inference_block_offload_device = device
        self.inference_block_offload_storage_device = offload_device

        self.input_layer.to(device)
        self.t_embedder.to(device)
        self.out_layer.to(device)
        if self.share_mod:
            self.adaLN_modulation.to(device)
        for name in ("pos_emb", "rope_phases"):
            buf = getattr(self, name, None)
            if isinstance(buf, torch.Tensor):
                setattr(self, name, buf.to(device))
        self.blocks.to(offload_device)

    def disable_inference_block_offload(self) -> None:
        self.inference_block_offload = False
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor, cond: torch.Tensor) -> torch.Tensor:
        ...
        block_offload = (
            getattr(self, "inference_block_offload", False)
            and not self.training
            and not torch.is_grad_enabled()
        )
        if block_offload:
            offload_device = getattr(self, "inference_block_offload_storage_device", torch.device("cpu"))
            for block in self.blocks:
                block.to(h.device)
