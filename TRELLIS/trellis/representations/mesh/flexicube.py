...
class FlexiCubes:
    ...
    def _compute_vd(self, voxelgrid_vertices, surf_cubes_fx8, surf_edges, scalar_field,
        ...
        vd = torch.zeros((total_num_vd, 3), device=self.device, dtype=voxelgrid_vertices.dtype)
        beta_sum = torch.zeros((total_num_vd, 1), device=self.device, dtype=scalar_field.dtype)
