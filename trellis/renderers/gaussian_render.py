...
def render(viewpoint_camera, pc : Gaussian, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None):
    ...
    # Detach and clone tensors to ensure they're not connected to autograd graph
    rendered_image, radii = rasterizer(
        means3D = means3D.detach().clone() if means3D is not None else None,
        means2D = means2D.detach().clone() if means2D is not None else None,
        shs = shs.detach().clone() if shs is not None else None,
        colors_precomp = colors_precomp.detach().clone() if colors_precomp is not None else None,
        opacities = opacity.detach().clone() if opacity is not None else None,
        scales = scales.detach().clone() if scales is not None else None,
        rotations = rotations.detach().clone() if rotations is not None else None,
        cov3D_precomp = cov3D_precomp.detach().clone() if cov3D_precomp is not None else None
    )
