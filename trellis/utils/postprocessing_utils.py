...
def postprocess_mesh(
    ...
    # Ensure vertices and faces are in the correct dtype for pyvista
    vertices = vertices.astype(np.float32)
    faces = faces.astype(np.int32)

    # Simplify
    if simplify and simplify_ratio > 0:
        mesh = pv.PolyData(vertices, np.concatenate([np.full((faces.shape[0], 1), 3, dtype=np.int32), faces], axis=1))
...
def bake_texture(
    ...
    Args:
        ...
        verbose (bool): Whether to print progress.
    """
    # nvdiffrast requires fp32 for rasterization, but use fp16 for observations to save memory
    ...
    views = [utils3d.torch.extrinsics_to_view(torch.tensor(extr, dtype=torch.float32).cuda()) for extr in extrinsics]
    projections = [utils3d.torch.intrinsics_to_perspective(torch.tensor(intr, dtype=torch.float32).cuda(), near, far) for intr in intrinsics]

    if mode == 'fast':
        # Use fp32 for accumulation to avoid precision loss, convert observations to fp32 as well
        ...
        for observation, view, projection in tqdm(zip(observations, views, projections), total=len(observations), disable=not verbose, desc='Texture baking (fast)'):
            ...
            # Convert observations to fp32 for accumulation
        ...
        # texture is already fp32, convert to uint8 numpy array
        ...
        # nvdiffrast requires fp32 for texture operations
...
def to_glb(
    ...
    # Clear CUDA cache before memory-intensive texture baking
